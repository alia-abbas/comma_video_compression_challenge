class JointFrameGenerator(nn.Module):
    ...
    def generate_single_frame(self, mask, pose, time_idx):
        ...
        # 1. Force mask to be 3D [Batch, H, W]
        # This removes any nested time/batch dims like [1, 1, H, W]
        if mask.dim() > 3:
            mask = mask.squeeze()
            if mask.dim() == 2: # If it squeezed TOO much
        ...
        # 2. Embedding call
        x = self.embedding(mask.long()) # Should produce [B, H, W, C]
        
        # --- THE NUCLEAR FIX ---
        # If x is 5D [B, 1, H, W, C], collapse the middle dim
        if x.dim() == 5:
            x = x.view(b, h, w, -1) 
        
        # Now permute is safe: [B, C, H, W]
        x = x.permute(0, 3, 1, 2)
        
        # 3. Concatenate and continue
...
class NeRVGenerator(nn.Module):
    def __init__(self, num_frames, embed_dim=64, hidden_dim=128):
        super().__init__()
        # 1. The "Codebook" - stores the temporal dynamics (movement)
        self.time_embed = nn.Embedding(num_frames, embed_dim)
        
        # 2. The "Decoder" - 1x1 convolutions acting as a per-pixel MLP
        # Input: 2 (x,y coords) + embed_dim -> Output: 3 (RGB)
    ...
    def forward(self, frame_idx, target_h, target_w):
        """
        frame_idx: Tensor of shape [B] containing the current frame numbers.
        """
        b = frame_idx.shape[0]
        device = frame_idx.device
        
        # Get the temporal code for this specific frame: [B, embed_dim]
        t_emb = self.time_embed(frame_idx) 
        
        # Reshape to broadcast across the spatial dimensions: [B, embed_dim, target_h, target_w]
        t_emb = t_emb.view(b, -1, 1, 1).expand(-1, -1, target_h, target_w)
        
        # Create the (x, y) coordinate grid: [B, 2, target_h, target_w]
        coords = self.make_grid(b, target_h, target_w, device)
        
        # Concatenate Time + Space: [B, 2 + embed_dim, H, W]
        # This is your (x, y, t) vector for every single pixel simultaneously!
        x = torch.cat([coords, t_emb], dim=1)
        
        # Map coordinates & time to RGB
