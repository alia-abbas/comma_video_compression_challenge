...
SALIENCY_DIR = REPO / 'submissions/root/saliency'
...
def posenet_grad(frame_t_np, frame_t1_np, posenet, device):
    ...
    """
    x0 = to_tensor(frame_t_np,  device).requires_grad_(True)
    x1 = to_tensor(frame_t1_np, device).requires_grad_(True)
    ...
    grad0 = x0.grad.abs().sum(dim=[0, 1])       # (H, W)
    grad1 = x1.grad.abs().sum(dim=[0, 1])       # (H, W)
    return grad0, grad1
